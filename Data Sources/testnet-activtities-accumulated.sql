...
all_users as (
  ...
  FROM   
    (SELECT
      ...
      AVG(tx_gas_price/pow(10,9)) as avg_gas_price --Gwei
...
SELECT
  ...
  AVG(new_users_percent) as avg_new_users_percent,
  AVG(retention_ratio) as avg_retention,
  SUM(total_gas_spent) as total_gas_used,
  SUM(total_gas_spent/transactions) as avg_gas_spent,
  AVG(avg_gas_price) as avg_avg_gas_price
FROM
  final_tab
GROUP BY chain_name
ORDER BY chain_name
